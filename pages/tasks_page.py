...
STYLES = {
    ...
    "btn_primary": """
        ...
        QPushButton:hover {
            background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 #2b6cb0, stop:1 #38b2ac);
        }
        QPushButton:pressed { margin-top: 2px; }
    """,
    "btn_secondary": """
        QPushButton {
            background-color: white; color: #4a5568; border: 1px solid #cbd5e0; 
            border-radius: 12px; padding: 8px 16px; font-weight: 600;
        }
        QPushButton:hover { background-color: #f7fafc; color: #2d3748; border-color: #a0aec0; }
    """
}

# =============================================================================
# 2. DIALOGUE D'AJOUT / ÉDITION
# =============================================================================
class AddTaskDialog(QDialog):
    def __init__(self, parent=None, title_text="New Task"):
        ...
        lbl_head.setStyleSheet("color: #3182ce; font-size: 12px; font-weight: 800; letter-spacing: 1px;")
        ...
        # Row for Date (DEADLINE) & Priority
        lbl_due = QLabel("Deadline:")
        lbl_due.setStyleSheet("font-size: 11px; color: #718096; margin-bottom: 2px;")
        ...
        self.priority_input.addItem("⬇️ Too Low Impact", "too low")
        self.priority_input.addItem("⚪ Low Impact", "low")
        self.priority_input.addItem("🔶 Medium Priority", "medium")
        self.priority_input.addItem("🔥 High Priority", "high")
...
# =============================================================================
# 3. DIALOGUE DE VUE (DETAILS)
# =============================================================================
class ViewTaskDialog(QDialog):
    def __init__(self, title, desc, due_date, created_date, priority, parent=None):
        super().__init__(parent)
        ...
        t = QLabel(title)
        t.setWordWrap(True)
        t.setStyleSheet("font-size: 22px; font-weight: 800; color: #1a202c; margin-top: 5px;")
        layout.addWidget(t)
        ...
        # Desc
        desc_box = QFrame()
        desc_box.setStyleSheet("background: #f7fafc; border-radius: 10px; padding: 15px;")
        dl = QVBoxLayout(desc_box)
        dl.setContentsMargins(0,0,0,0)
        lbl_desc = QLabel(desc if desc else "No details provided.")
        lbl_desc.setWordWrap(True)
        lbl_desc.setStyleSheet("color: #4a5568; line-height: 1.4;")
...
# =============================================================================
# 4. CARTE DE TÂCHE (DESIGN AVEC 2 DATES)
# =============================================================================
class TaskCard(QFrame):
    def __init__(self, t_id, title, desc, due_date_pretty, created_date_pretty, priority, parent_page):
        ...
        self.badge.setStyleSheet(f"""
            background: {bg_style}; 
            color: white; border-radius: 10px; font-size: 9px; font-weight: 900;
        """)
        ...
        self.lbl_title.setStyleSheet("color: #2d3748; font-size: 15px; font-weight: 700; border: none;")
        layout.addWidget(self.lbl_title)

        # 3. Footer (Dates + Actions)
        footer = QHBoxLayout()
        
        # Section des dates (Verticale)
        dates_layout = QVBoxLayout()
        dates_layout.setSpacing(2)
        
        # Date de création
        lbl_created = QLabel(f"🌱 {created_date_pretty}")
        lbl_created.setStyleSheet("color: #a0aec0; font-size: 10px; border: none;")
        
        # Date d'échéance (Deadline)
        lbl_due = QLabel(f"⏰ {due_date_pretty}")
        lbl_due.setStyleSheet("color: #e53e3e; font-size: 11px; font-weight: 700; border: none;")
    ...
    def on_checked(self, checked):
        ...
        self.lbl_title.setStyleSheet(f"color: {'#cbd5e0' if checked else '#2d3748'}; font-size: 15px; font-weight: 700; border: none;")
...
# =============================================================================
# 5. COLONNE JOURNALIÈRE
# =============================================================================
class DayColumn(QWidget):
    def __init__(self, title, is_today=False):
        super().__init__()
        self.setFixedWidth(300)
        layout = QVBoxLayout(self)
        layout.setContentsMargins(5, 0, 15, 0)
        
        head_color = "#3182ce" if is_today else "#718096"
        lbl = QLabel(title)
        lbl.setStyleSheet(f"font-size: 16px; font-weight: 800; color: {head_color}; margin-bottom: 10px;")
...
class TasksPage(QWidget):
    def __init__(self):
        ...
        welcome.setStyleSheet("font-size: 12px; font-weight: 800; color: #3182ce; text-transform: uppercase; letter-spacing: 2px;")
        
        title = QLabel("Your Creative Flow")
        title.setStyleSheet("font-size: 32px; font-weight: 900; color: #1a202c;")
        
        txt_layout.addWidget(welcome)
        ...
        scroll.setStyleSheet("""
            ...
            QScrollBar:horizontal { height: 8px; background: #edf2f7; border-radius: 4px; }
    ...
    def init_db(self):
        ...
        # 2. MIGRATION : Ajouter la colonne created_date si elle manque (pour les anciens utilisateurs)
        try:
            conn.execute("ALTER TABLE tasks ADD COLUMN created_date TEXT")
    ...
    def show_task_details(self, t_id):
        conn = self.get_db_connection()
        row = conn.execute("SELECT title, description, due_date, created_date, priority FROM tasks WHERE id=?", (t_id,)).fetchone()
        conn.close()
        # Si created_date est vide (anciennes tâches), on met un texte par défaut
        created = row[3] if row[3] else "Unknown"
        if row: ViewTaskDialog(row[0], row[1], row[2], created, row[4], self).exec()

    def edit_task(self, t_id):
        conn = self.get_db_connection()
        row = conn.execute("SELECT title, description, due_date, priority FROM tasks WHERE id=?", (t_id,)).fetchone()
        conn.close()
        if row:
            dlg = AddTaskDialog(self, "Edit Task")
            dlg.load_data(row[0], row[1], row[2], row[3])
            if dlg.exec():
                data = dlg.get_data()
  ...
  # --- Helpers BDD ---
    ...
    def refresh_tasks(self):
        ...
        conn = self.get_db_connection()
        try:
            rows = conn.execute("SELECT id, title, description, due_date, created_date, priority FROM tasks WHERE is_completed=0 ORDER BY due_date").fetchall()
        except:
            rows = []
        conn.close()
        ...
        map_cols = {}
        for row in rows:
            t_id, title, desc, due_date_str, created_date_str, priority = row
            
            # --- C'EST ICI QUE L'ON AJOUTE L'ANNÉE ---
            
            # 1. Date d'échéance (Deadline) avec l'année
            if not due_date_str:
                pretty_due = "No Deadline"
            else:
                qdate = QDate.fromString(due_date_str, "yyyy-MM-dd")
                # Ajout de 'yyyy' pour l'année complète (ex: 2026)
                pretty_due = qdate.toString("dddd d MMMM yyyy") 
            
            # 2. Date de création (plus court, mais avec année aussi)
            if not created_date_str:
                pretty_created = "Unknown"
            else:
                qdate_c = QDate.fromString(created_date_str, "yyyy-MM-dd")
                pretty_created = qdate_c.toString("d MMM yyyy") 

            # Création de la colonne si elle n'existe pas encore pour cette date
            if pretty_due not in map_cols:
                is_today = (due_date_str == QDate.currentDate().toString("yyyy-MM-dd"))
                col = DayColumn(pretty_due, is_today)
